...
- **B2** 〔自由交互①〕点 Acme → focus 动画（相关亮 / 无关淡），建立 `Project→Task→Employee` 结构感。**此处不 drill**——详情页 drill 一律移到 Nexus reveal（B6）之后。理由：详情页**静态恒显全部模块**（含 agent 智能层），若 agent 没跑就 drill 进项目页会提前剧透 killer beat。见 [ADR-0004](../adr/0004-detail-pages-static-reveal-protected-by-beat-order.md)。
...
- **B8** 〔capability〕agent 调 tool → 造 **timeline 图**：主方案**保周五**（interrupt 甩 Kai + 砍 dedupe），Tue slip 标 **conditional 兜底**（绑定 rate-limit 变数）。
