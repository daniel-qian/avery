...
// P3-02：项目详情页 = 静态恒显（ADR-0005，零 thread 耦合）。是 B9b（drill 回 Acme 看 handoffs 落地）的页面。
...
export function ProjectDetailScene() {
  ...
  const handoffs = handoffsForProject(project.id)
  const weekly = weeklyUpdatesForProject(project.id)
  const signals = signalsForProject(project.id)
